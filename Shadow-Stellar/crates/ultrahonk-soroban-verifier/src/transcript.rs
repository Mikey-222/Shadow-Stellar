...
pub fn generate_transcript(
    ...
    trace!("eta = 0x{:x}", crate::debug::Hex(&rp.eta.to_bytes()));
    trace!("eta_two = 0x{:x}", crate::debug::Hex(&rp.eta_two.to_bytes()));
    trace!("eta_three = 0x{:x}", crate::debug::Hex(&rp.eta_three.to_bytes()));
    trace!("beta = 0x{:x}", crate::debug::Hex(&rp.beta.to_bytes()));
    trace!("gamma = 0x{:x}", crate::debug::Hex(&rp.gamma.to_bytes()));
    trace!("rho = 0x{:x}", crate::debug::Hex(&rho.to_bytes()));
    trace!("gemini_r = 0x{:x}", crate::debug::Hex(&gemini_r.to_bytes()));
    trace!("shplonk_nu = 0x{:x}", crate::debug::Hex(&shplonk_nu.to_bytes()));
    trace!("shplonk_z = 0x{:x}", crate::debug::Hex(&shplonk_z.to_bytes()));
